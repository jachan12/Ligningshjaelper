...
function formatNumber(n) {
  // Vis altid max 2 decimaler, brug komma som decimaltegn
  if (typeof n !== "number") return n;
  if (Number.isInteger(n)) return n.toString();
  // Afrund til 2 decimaler og brug komma
  return n.toFixed(2).replace(".", ",");
}

// Hjælpefunktion til at afrunde til 2 decimaler og bruge det i beregninger
function round2(n) {
...
function generateStepExplanation(inputStr, beforeEq, afterEq) {
  // Returnér kun regnestykket hvor operationen sker
  ...
  if (operation === "+" || operation === "-") {
    regneStykke = `${beforeSides[0]}${opSymbol} = ${beforeSides[1]}${opSymbol}`;
  } else if (operation === "*" || operation === "/") {
    // Gør det på ALLE led, ikke udenfor parentes
    // Split venstre og højre side op i led (brug + og - som split)
    ...
    const leftTerms = splitTerms(beforeSides[0]);
    const rightTerms = splitTerms(beforeSides[1]);
    // Gange/dividere hvert led
    const leftNew = leftTerms.map(t => `${t}${opSymbol}`).join(" + ").replace(/\+\s*-/g, "- ");
    const rightNew = rightTerms.map(t => `${t}${opSymbol}`).join(" + ").replace(/\+\s*-/g, "- ");
    regneStykke = `${leftNew} = ${rightNew}`;
  }
  let explanation = `<b>${regneStykke}</b>`;
...
function applyStep() {
  ...
  if (inputStr.toLowerCase() === "sqrt") {
    if (
      ...
      // Hele trinnet (li) bliver nu et <details> med <summary>
      const details = document.createElement("details");
      details.className = "step-dropdown";
      const summary = document.createElement("summary");
      summary.innerHTML = historyEntry.innerHTML;
      const exp = document.createElement("div");
      exp.className = "step-explanation-inline";
      exp.innerHTML = generateStepExplanation(inputStr, before, formatEquation());
      // Dropup: explanation først, så summary bagefter
      details.appendChild(exp);
      details.appendChild(summary);
      // Udskift historyEntry med details
      if (isSolved) {
        details.style.background = "#c8ffc8";
        summary.innerHTML += " ✔️";
      }
  ...
  historyEntry.innerHTML = formatEquation() + `   ( ${operation}${isXOp ? (factor === 1 && pow === 1 ? "x" : formatNumber(factor) + (pow > 1 ? "x^" + pow : "x")) : formatNumber(step)} på begge sider)`;
...
function toNumber(val) {
  return isFraction(val) ? round2(val.num / val.den) : round2(val);
}
function addFactors(a, b) {
  ...
  // For decimaltal: afrund til 2 decimaler
  return round2(a + b);
}
